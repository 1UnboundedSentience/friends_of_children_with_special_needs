...
# The data can then be loaded with the rake db:seed (or created alongside the db with db:setup).
  ...
  coordinator_art_attrs = emergency_contact_attrs.merge(email: 'art_coordinator@fcsn.org')

  coordinator_person = Person.create_with(coordinator_attrs).find_or_create_by(email: coordinator_attrs[:email])
  coordinator_art_person = Person.create_with(coordinator_art_attrs).find_or_create_by(email: coordinator_art_attrs[:email])
  ...
  coordinator_art = User.create_with(is_coordinator: true, password: 'testing123', person: coordinator_art_person)
                    .find_or_create_by(email: coordinator_art_person[:email])
